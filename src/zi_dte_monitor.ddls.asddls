  ...
  composition [0..*] of ZI_DTE_MONITOR_H as _Historial
{
  ...
  key m.proveedor                   as Proveedor,
      ...
      case m.estado
        when '01' then cast( 0 as abap.int1 )  " Pendiente   → None / Gris
        when '02' then cast( 3 as abap.int1 )  " Validado    → Success / Verde
        when '03' then cast( 1 as abap.int1 )  " Rechazado   → Error / Negro (vía formatter)
        when '04' then cast( 2 as abap.int1 )  " Por Rechazar→ Warning / Naranja
        when '05' then cast( 1 as abap.int1 )  " No Procesado→ Error / Rojo
        when '06' then cast( 5 as abap.int1 )  " Aceptado    → Information / Azul
